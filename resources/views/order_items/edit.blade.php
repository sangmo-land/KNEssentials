    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                <form method="POST" action="{{ route('order_items.update', $orderItem->id) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-4">
                        ...
                        <select id="order_id" name="order_id"
                            ...
                            <option value="{{ $order->id }}" {{ $order->id == $orderItem->order_id ? 'selected' : ''
                                }}>Order #{{ $order->id }} - {{ $order->user->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" id="quantity" name="quantity" min="1" value="{{ $orderItem->quantity }}"
                            class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                            required>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" step="0.01" id="price" name="price" value="{{ $orderItem->price }}"
                            class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                            required>
                    </div>
